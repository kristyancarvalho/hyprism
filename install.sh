...
verify_link() {
  local source=$1 destination=$2
  [[ -L $destination ]] || { printf 'Expected symlink was not created: %s\n' "$destination" >&2; exit 1; }
...
if ((dry_run == 0)); then
  verify_link "$repo_dir" "$target_home/.local/share/hyprism"
  verify_link "$repo_dir/config/hypr" "$target_home/.config/hypr"
  verify_link "$repo_dir/config/quickshell" "$target_home/.config/quickshell"
  [[ -f $target_home/.config/hypr/hyprland.lua ]] \
    || { printf 'Missing installed Hyprland Lua entrypoint.\n' >&2; exit 1; }
  [[ ! -e $target_home/.config/hypr/hyprland.conf ]] \
    || { printf 'Deprecated Hyprland entrypoint is still installed.\n' >&2; exit 1; }
  [[ -f $target_home/.config/quickshell/shell.qml ]] \
    || { printf 'Missing installed Quickshell entrypoint.\n' >&2; exit 1; }
fi

# This file was generated by older Hyprism releases. Preserve it in the same
# timestamped backup tree rather than allowing it to be sourced accidentally.
...
if [[ -f $legacy_hypr_theme ]]; then
  ...
  printf 'Archived obsolete generated Hyprland config at %s\n' "$legacy_backup"
fi
...
missing=()
for command in Hyprland quickshell qs kitty matugen awww wl-copy cliphist nmcli wpctl playerctl grim slurp; do command -v "$command" >/dev/null || missing+=("$command"); done
