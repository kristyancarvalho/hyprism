...
fallback={'city':'Weather unavailable','temperature':None,'apparentTemperature':None,'weatherCode':-1,'minimum':None,'maximum':None,'updatedAt':0}
...
try:
    ...
    result={'city':settings.get('location') or 'Local weather','temperature':data['current']['temperature_2m'],'apparentTemperature':data['current']['apparent_temperature'],'weatherCode':data['current']['weather_code'],'minimum':data['daily']['temperature_2m_min'][0],'maximum':data['daily']['temperature_2m_max'][0],'updatedAt':int(time.time())}
