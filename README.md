...
O blur fica somente sob a senha. A ação de login, mostrar senha, teclado, energia e os perfis de sessão são botões de ícone com tooltips curtos. O seletor de sessão cria seus itens a partir do modelo exposto pelo SDDM: a sessão ativa recebe o acento `primary`, e seu nome somente acompanha o ícone quando selecionada, em foco ou hover. `SessionIdentity.qml` normaliza nomes e associa Hyprland, dwm, KDE Plasma, GNOME, XFCE, Sway, i3, Qtile, Awesome, bspwm, Cinnamon, MATE, LXDE e LXQt aos glifos oficiais presentes no Symbols Nerd Font; dwl e sessões desconhecidas usam o fallback visual de sessão sem impedir a seleção. O teclado virtual reutiliza `QtQuick.VirtualKeyboard.InputPanel` do fork, provisionado por `qt6-virtualkeyboard`, enquanto o drop-in define `InputMethod=qtvirtualkeyboard` e `QT_IM_MODULE=qtvirtualkeyboard`. O botão compacto de teclado alterna o painel real e devolve foco à senha.
