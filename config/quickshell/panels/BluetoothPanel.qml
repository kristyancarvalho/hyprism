...
Item {
    ...
    Column { anchors.fill: parent; anchors.margins: 18; spacing: 10
        Text { text: !controller.system.bluetooth.available ? "Bluetooth indisponível" : "Bluetooth"; color: panel.theme.colors.foreground; font { pixelSize: 17; bold: true } }
        Button { visible: controller.system.bluetooth.available; text: controller.system.bluetooth.powered ? "Desligar Bluetooth" : "Ligar Bluetooth"; onClicked: panel.command(["python3", controller.rootDir + "/scripts/system/bluetooth.py", "power", controller.system.bluetooth.powered ? "off" : "on"]) }
        Repeater { model: controller.system.bluetooth.devices
            Rectangle { required property var modelData; width: panel.width - 36; height: 52; radius: 10; color: panel.theme.colors.surfaceVariant
                ...
                Button { id: action; anchors { right: parent.right; verticalCenter: parent.verticalCenter; rightMargin: 5 } text: modelData.connected ? "Desconectar" : "Conectar"; onClicked: panel.command(["python3", controller.rootDir + "/scripts/system/bluetooth.py", modelData.connected ? "disconnect" : "connect", modelData.address]) }
            }
        }
